...
mount(function () {
    ...
    if (Auth::user()->hasRole('user')) {
        ...
        $this->total_files = Assignment::where('user_id', Auth::user()->id)->count();
    }

    if (Auth::user()->can('users.index')) {
        $this->total_users = User::all()->count();
        $this->total_credit = Balance::sum('total_value');
        $this->total_files = Assignment::all()->count();
    }
...
<div x-data="{ modalOpen: false, modalApproveOpen: false, pack: 0 }"
     @keydown.escape.window="modalOpen = false; modalApproveOpen = false"
     x-on:close-modal.window="modalOpen = false; modalApproveOpen = false"
     x-on:open-modal.window="modalApproveOpen = true">
    ...
    <div class="mb-4">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 flex flex-col gap-3 items-center justify-center">
                <svg class="w-12 h-12 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                     stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round"
                          d="M15.182 16.318A4.486 4.486 0 0 0 12.016 15a4.486 4.486 0 0 0-3.198 1.318M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0ZM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Z"/>
                </svg>
                <p class="text-center"><span class="text-lg">Your current balance is <b>"0"</b> </span><br/> <span
                        class="text-md text-gray-600">charge your balance in order to create new assignments</span></p>
            </div>
        </div>
    </div>
    @endrole
                ...
                <table>
                    ...
                    @foreach($assignments as $row)
                        <tr>
                            ...
                            <td class="md:w-[300px] @if($row->status  === BalanceLineStatus::APPROVED) text-green-700 @elseif($row->status  === BalanceLineStatus::PENDING) text-gray-600 @else line-through @endif"><b><span  class="@if($row->status  != BalanceLineStatus::APPROVED) invisible @endif">+ </span>{{ number_format($row->value, 2)  }}</b></td>
                            <td>{{ $row->created_at->format('Y-m-d H:i') ?? '-' }}</td>
                            <td>
                                <div class="inline-flex align-items-center justify-center px-3 py-2 rounded-md
                                {{ $row->status === BalanceLineStatus::APPROVED ? 'bg-emerald-300' : ($row->status === BalanceLineStatus::REFUSED ? 'bg-red-300' : 'bg-orange-300' ) }} font-weight-bolder text-xs">
                                    {{ $row->status }}
                                </div>
                            </td>
                            <td>
        ...
        <div x-show="modalApproveOpen"
            ...
            <div x-show="modalApproveOpen"
                ...
                <div class="flex items-center justify-between pb-2">
                    ...
                    <button @click="modalApproveOpen=false"
